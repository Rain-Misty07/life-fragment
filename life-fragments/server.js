...
app.post("/api/login", async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      "SELECT id, account, password, nickname, signature FROM users WHERE account = ? LIMIT 1",
      [account]
    );
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (signature.length > 128) {
      return res.status(400).json({ success: false, message: "个性签名不能超过 128 个字符" });
    }
    ...
    await pool.execute(
      "INSERT INTO users (account, password, nickname, signature) VALUES (?, ?, ?, ?)",
      [account, hash, nickname, signature]
    );

    res.status(201).json({
      success: true,
      user: { account, nickname, signature },
    });
  } catch (err) {
...
async function fetchEntriesByUserId(userId, { publicOnly = false } = {}) {
  let sql = `SELECT id, content, mood, is_private, created_at, updated_at
     FROM entries
     WHERE user_id = ?`;
  if (publicOnly) {
...
app.get("/api/entries", async (req, res) => {
  try {
    ...
    const publicOnly = viewerAccount !== account;
    const entries = await fetchEntriesByUserId(access.user.id, { publicOnly });
...
app.get("/api/friends/entries", async (req, res) => {
  try {
    ...
    const entries = await fetchEntriesByUserId(access.user.id, { publicOnly: true });
    res.json({ success: true, entries });
  } catch (err) {
...
app.post("/api/entries", async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      "INSERT INTO entries (user_id, content, mood, is_private) VALUES (?, ?, ?, ?)",
      [user.id, content, mood, isPrivate ? 1 : 0]
    );

    const [rows] = await pool.execute(
      "SELECT id, content, mood, is_private, created_at, updated_at FROM entries WHERE id = ? LIMIT 1",
      [result.insertId]
    );
...
app.put("/api/entries/:id", async (req, res) => {
  try {
    ...
    await pool.execute(
      "UPDATE entries SET content = ?, mood = ?, is_private = ? WHERE id = ? AND user_id = ?",
      [content, mood, isPrivate ? 1 : 0, entryId, user.id]
    );
...
async function getEntryOwnerAccount(entryId) {
  const [rows] = await pool.execute(
    `SELECT u.account, u.id AS owner_id, e.is_private
     FROM entries e
...
async function assertCanCommentOnEntry(viewerAccount, entryId) {
  ...
  if (entryOwner.is_private && viewerAccount !== entryOwner.account) {
    return { ok: false, status: 403, message: "该随记为私密，仅作者可见" };
  }
...
app.put("/api/profile", async (req, res) => {
  try {
    ...
    if (password) {
      const hash = await bcrypt.hash(password, 10);
      await pool.execute(
        "UPDATE users SET nickname = ?, signature = ?, password = ? WHERE account = ?",
        [nickname, signature, hash, account]
      );
    } else {
      await pool.execute(
        "UPDATE users SET nickname = ?, signature = ? WHERE account = ?",
        [nickname, signature, account]
      );
    }
